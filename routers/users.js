...
import { checkUsers, getHashedPassword, getUsers, postUser } from "../helper/Helper.js";
...
router.post("/add-user", async (request, response) => {
  const { username, email, phone, type, password } = request.body;

  if (!username || !email || !password || !type || !password) {
    return response.status(400).send({ message: "All fields are mandatory" });
  }

  const hashedPassword = await getHashedPassword(password);
  const users = await checkUsers(username, email);
  ...
  if (
    ...
    response.status(400).send({
      message:
        "Password must be 8 characters long and contain atleast one Uppercase, one Lowercase, one number and one special character",
    });
    return;
  }
  const result = await postUser({
    ...
    email,
    phone,
    type: type === "admin" ? type : null,
    password: hashedPassword,
  });
  result.insertedId
    ? response.status(200).send({ message: "User added successfully" })
    : response.status(400).send({ message: "Failed to add user" });
});
